...
(with-eval-after-load 'org
  ...
  ;; Function "bh/remove-empty-drawer-on-clock-out" locates at funcs.el
...
as the default task."
  ...
  ;;
  ;; (require 'org-id) ;; Moved to package.el
...
Late deadlines first, then scheduled, then non-late deadlines"
  ...
  ;; (require 'org-checklist)  ;; cyd@20160730 Moved to package.el
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  ;;
  ;; I enter notes for tasks with C-c C-z (or just z in the agenda). Changing
  ;; tasks states also sometimes prompt for a note (e.g. moving to WAITING
  ;; prompts for a note and I enter a reason for why it is waiting). These notes
  ;; are saved at the top of the task so unfolding the task shows the note first.
  ...
  ;; org-yank-adjust-subtrees) for more details on each variable and what it does.
  ;;
  ;; I have org-special-ctrl-a/e set to enable easy access to the beginning and
  ;; end of headlines. I use M-m or C-a C-a to get to the beginning of the line
  ;; so the speed commands work and C-a to give easy access to the beginning of
  ;; the heading text when I need that.
  ...
  ;;    DEADLINE: <2009-07-01 Wed +1m -0d>
  ;;
  ;; This example repeats monthly and shows up in the agenda on the day it is
  ;; due (with no prior warning). You can set any number of lead days you want on
  ...
  ;; hit M-x org-table-export which prompts for a filename and the format which
  ;; defaults to orgtbl-to-csv from the setting above.
  ...
  ;; the current window for C-c ' source editing
  ...
  ;;
  (setq org-clock-sound (concat audioPath "01.RoosterAlarm.wav") )
  ...
  ;; org-files. The following setting keeps the highlighted results of the
  ;; search even after modifying the text. This allows me to edit the file
  ;; without having to reissue the org-occur command to find the other matches
  ;; in my file. C-c C-c removes the highlights.
  ...
  ;; the default setting. I've been using regular M-x occur a lot more lately to
  ;; find things in any Emacs buffer.
  ...
  ;; search with / TAB SomeTag the agenda remembers this filter until you change it.
  ;;
  ...
  ;; Do not write any code after this comments
